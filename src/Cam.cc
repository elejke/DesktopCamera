...
class MapStorage {
 public:
  Mat color_map;
  int getWidth() { return color_map.cols; }
  int getHeight() { return color_map.rows; }
  double getvalue();
};
...
double MapStorage::getvalue() {
  ...
  // cout<<width<<" "<<height<<" "<<top<<" "<<bottom<<" "<<left<<"
  // "<<right<<endl;
...
Cam::Cam(const Napi::CallbackInfo& info) : Napi::ObjectWrap<Cam>(info) {
  this->now = -1;
...
Napi::Value Cam::UpdateImage(const Napi::CallbackInfo& info) {
  ...
  task_runner = new std::thread([this, DebugMode, DotNetMode]() {
    ...
    while (true) {
      ...
      if ((x->color_map).empty()) {
        ...
        if (DotNetMode) {
          ...
          cv::putText((x->color_map), text, origin, font_face, font_scale,
